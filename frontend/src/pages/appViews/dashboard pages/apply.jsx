...
const JobCard = ({ job, loggedUser, refreshJobs }) => {
  ...
  return (
    <li className="bg-white p-4 rounded-lg shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {job.category && Array.isArray(job.category) && job.category.length > 0 ? (
              job.category.map((cat, idx) => (
                <span
                  key={idx}
                  className={`bg-${cat.color}-100 text-${cat.color}-500 px-2 py-1 rounded-full text-xs`}
                >
                  {cat.label}
                </span>
              ))
      ...
      <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
        <button
          onClick={handleApplyClick}
          className="bg-[rgba(239,146,115,1)] text-white px-4 py-2 rounded-md hover:bg-[rgba(239,146,115,0.9)] transition whitespace-nowrap"
        >
          {loggedUser?.role === 'jobseeker' ? 'Apply' : 'View Details'}
        </button>
        {!job?.examSet && loggedUser?.role === 'recruiter' && (
          <>
            <button
              onClick={() => setShowExamModal(true)}
              className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md transition whitespace-nowrap"
            >
              Set Exam
            </button>
            <button
              onClick={() => setShowTaskModal(true)}
              className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-md transition whitespace-nowrap"
            >
              Add Files
            </button>
          </>
...
const Apply = () => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <main className="max-w-6xl mx-auto px-4 py-4 h-[calc(100vh-80px)] overflow-y-auto">
        ...
        <div className="bg-white flex flex-col md:flex-row items-center justify-between p-4 rounded-lg shadow mb-8 gap-4">
          ...
          <div className="flex space-x-2 w-full md:w-auto">
            <button className="bg-[rgba(239,146,115,1)] hover:bg-[rgba(239,146,115,0.9)] text-white px-4 py-2 rounded transition whitespace-nowrap">
              Search Jobs
            </button>
            {loggedUser?.role !== "jobseeker" && (
              <Link 
                to="/dashboard/createjob"
                className="bg-[rgba(239,146,115,1)] hover:bg-[rgba(239,146,115,0.9)] text-white px-4 py-2 rounded transition flex items-center whitespace-nowrap"
              >
                Create Job
              </Link>
            )}
